...
public class WaterDM300 extends Mob {

	{
		spriteClass = WaterDM300Sprite.class;
				...
				if (enemy != null && !canReach){
					...
					if (turnsSinceLastAbility >= MIN_COOLDOWN){
						...
						//if we can't gas, then drop rocks
							...
							if (sprite != null && (sprite.visible || enemy.sprite.visible)) {
								((WaterDM300Sprite)sprite).slam(enemy.pos);
								return false;
							} else {
								dropRocks(enemy);
	...
	public void move(int step) {
		...
		if (Dungeon.level.map[step] == Terrain.INACTIVE_TRAP && state == HUNTING) {
			...
			if (WaterBossLevel.WaterPylonEnergy.volumeAt(pos, WaterBossLevel.WaterPylonEnergy.class) > 0){
				return;
			}
	...
	public void dropRocks( Char target ) {
		...
		do {
			safeCell = rockCenter + PathFinder.NEIGHBOURS8[Random.Int(8)];
				...
				|| (Blob.volumeAt(safeCell, WaterBossLevel.WaterPylonEnergy.class) > 0 && Random.Int(2) == 0));
	...
	public void supercharge(){
		supercharged = true;
		((WaterBossLevel)Dungeon.level).activatePylon();
		pylonsActivated++;

		spend(3f);
		yell(Messages.get(this, "charging"));
		sprite.showStatus(CharSprite.POSITIVE, Messages.get(this, "invulnerable"));
		((WaterDM300Sprite)sprite).updateChargeState(true);
		((WaterDM300Sprite)sprite).charge();
	...
	public void loseSupercharge(){
		supercharged = false;
		((WaterDM300Sprite)sprite).updateChargeState(false);

...
import com.elementalpixel.elementalpixeldungeon.effects.particles.BlastParticle;
import com.elementalpixel.elementalpixeldungeon.effects.particles.FlameParticle;
import com.elementalpixel.elementalpixeldungeon.effects.particles.SmokeParticle;
...
import com.elementalpixel.elementalpixeldungeon.sprites.TenguSprite;
...
public class AirTengu extends Mob {
			...
			if (abilityToUse != lastAbility || Random.Int(10) == 0){
				switch (abilityToUse){
					...
					case SHOCKER_ABILITY:
						abilityUsed = throwShocker(AirTengu.this, enemy);
			...
			if (timer == 3) {
				...
				for (int cell = 0; cell < PathFinder.distance.length; cell++) {
					...
					if (PathFinder.distance[cell] < Integer.MAX_VALUE) {
						...
						if (ch != null && !(ch instanceof AirTengu)) {
	...
	public static boolean throwFire(final Char thrower, final Char target){
		...
		for (int i = 0; i < PathFinder.CIRCLE8.length; i++){
			if (aim.sourcePos+PathFinder.CIRCLE8[i] == aim.path.get(1)){
				...
				Buff.append(thrower, AirTengu.FireAbility.class).direction = i;
				...
				for (int i = area.left; i < area.right; i++){
					for (int j = area.top; j < area.bottom; j++){
						...
						if (cur[cell] > 0 && off[cell] == 0){
							...
							Char ch = Actor.findChar( cell );
							if (ch != null && !ch.isImmune(Fire.class) && !(ch instanceof AirTengu)) {

...
public class InfernalFlame extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (target.isAlive() && !target.isImmune(getClass())) {
			...
			int damage = Random.NormalIntRange( 3, (int) Math.floor(Dungeon.depth * 0.567f));
			Buff.detach( target, Chill.class);
		...
		if (Dungeon.level.flamable[target.pos] && Blob.volumeAt(target.pos, InfernalFire.class) == 0) {
			GameScene.add( Blob.seed( target.pos, 4, InfernalFire.class ) );
		}
	...
	public void fx(boolean on) {
		if (on) target.sprite.add(CharSprite.State.INFERNAL_FLAME);
		else target.sprite.remove(CharSprite.State.INFERNAL_FLAME);
	}
